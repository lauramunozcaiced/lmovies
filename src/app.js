...
async function getMediaDetail(id) {
    ...
    const trailers = await getTrailerVideo(data.id, false);
    detailsVideos.innerHTML = '';
    if (trailers.length != 0) {
        trailers.slice(0, 2).forEach(video => {
            detailsVideos.innerHTML += `<iframe src="https://www.youtube.com/embed/${video.key}" width="560" height="400"frameborder="0"></iframe>`;
        })
    } else {
        detailsVideos.innerHTML = '<p>No trailers</p>';
    }
...
async function getTrailerVideo(id, isBanner = true, type = 'movie') {
